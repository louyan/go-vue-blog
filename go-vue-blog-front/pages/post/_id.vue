<template>
  <section class="post-section">
    ...
    <div class="metas">
      <nuxt-link :to="`/category/${post.category}`" class="cat-link">
      ...
      <span class="time">
        ...
        </svg>{{ post.create_time | formatTime }}
      </span>
    </div>
    <div class="markdown-body content" v-html="markdownContent">{{post.content}}</div>
    <div class="tags">
      <svg class="icon" aria-hidden="true">
        <use xlink:href="#icon-tag"></use>
      </svg>
      <nuxt-link class="tag" v-for="tag in tags" :key="tag.tagId" :to="`/search/tag/${tag.tagId}`">{{ tag.tag }}</nuxt-link>
    </div>
    <div class="prev-next">
      <p class="prev fl">
...
<script>
  ...
  export default {
    head () {
      return {
        title: `${this.post.title} - Powered by Louyan`
      };
    },
    validate ({ params }) {
      // Must be a number
    ...
    asyncData ({ params }) {
      return axios.get(`/article/getOne/${params.id}`).then(res => {
