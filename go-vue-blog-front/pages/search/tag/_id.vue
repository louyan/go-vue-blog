...
<script>
  ...
  export default {
    head () {
      return {
        title: `${this.tagName} - Powered by Louyan`
      };
    },
    components: {
      'post-list': Post,
    ...
    asyncData ({ params }) {
      const pageNum = 10;
      return axios.get(`/tag/${params.id}?page=1&pageNum=${pageNum}`).then(res => {
    ...
    methods: {
      loadMore: function () {
        if (!this.isNoMore) {
          ...
          axios.get(`/tag/${this.tagId}?page=${this.page}&pageNum=${this.pageNum}`).then(res => {
            if (res.data.success === 1) {
